...
CGINCLUDE
	...
	half4 frag( v2f i ) : SV_Target
	{
        ...
        //Вычисляем первую волну, которая параллельная берегу
        ...
        //Вычисляем вторую волну, используя текстурные координаты и искажение
        ...
        //Высчитываем порог ниже которого цвет волны будет отображаться. Используются горизонтальные координаты. Уменьшаем порог по мере удаления от берега.
        float threshold = positiveSin(i.bumpCoords.x * _TresholdDensity) * _MaxTreshold * (_TresholdFalloff - zDelta);
        //Прибавляем к финальному цвету цвет волны и цвет кромки у берега
